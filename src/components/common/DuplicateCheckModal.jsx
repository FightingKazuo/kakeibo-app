    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex flex-col justify-end">
      <div className="bg-white w-full rounded-t-3xl flex flex-col" style={{ maxHeight: "88vh" }}>
        ...
        <div className="p-5 pt-0 space-y-2 border-t border-gray-100">
          <button onClick={()=>onDecide("skip",candidates[idx])} className="w-full py-3.5 rounded-2xl border-2 border-rose-200 bg-rose-50 text-left px-4 active:scale-95">
            <p className="text-sm font-bold text-rose-700">☑ 同じ取引（重複）なので登録しない</p>
          </button>
          <button onClick={()=>onDecide("register",candidates[idx])} className="w-full py-3.5 rounded-2xl border-2 border-indigo-200 bg-indigo-50 text-left px-4 active:scale-95">
