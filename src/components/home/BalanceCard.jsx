...
export function BalanceCard({ totalIncome, totalExpense, thisMonthBalance, year, month }) {
  return (
    <div className="bg-gradient-to-br from-indigo-500 to-purple-600 px-6 pt-12 pb-10 text-white">
      <p className="text-sm opacity-80">{year}年{month}月の収支</p>
      <div className="text-center my-4">
        <p className="text-sm opacity-80 mb-1">累計残高</p>
      ...
      <div className="flex gap-3">
        <div className="flex-1 bg-white/20 rounded-2xl p-3">
          ...
          <p className="text-lg font-bold mt-1">{fmtCurrency(totalIncome)}</p>
        </div>
        <div className="flex-1 bg-white/20 rounded-2xl p-3">
          <p className="text-xs opacity-80">支出</p>
          <p className="text-lg font-bold mt-1">{fmtCurrency(totalExpense)}</p>
        </div>
      </div>
      {/* 今月残高 */}
      <div className={`mt-3 rounded-2xl p-3 flex items-center justify-between ${thisMonthBalance>=0?"bg-emerald-400/30":"bg-rose-400/30"}`}>
