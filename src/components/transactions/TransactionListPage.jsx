...
export function TransactionListPage({ transactions, categories, onEdit, onDelete, onNavigate }) {
    ...
    <div className="pb-20">
        ...
        <div className="flex gap-2 overflow-x-auto scrollbar-none">
          {[["all","すべて"],["manual","✏️手動"],["csv","📊CSV"],["ocr","📷OCR"]].map(([id,lb]) => (
            <button key={id} onClick={() => setSrcFilter(id)}
              className={`flex-shrink-0 px-3 py-1 rounded-full text-xs font-semibold border transition-all duration-200 ${srcFilter===id?"bg-gray-700 text-white border-gray-700":"bg-white text-gray-500 border-gray-200"}`}>
              {lb}
            </button>
          ))}
        </div>
      ...
      <div className="bg-white">
        {filtered.length===0 && transactions.length===0 ? (
          ...
          filtered.map(t => (
            <TransactionItem key={t.id} transaction={t} categories={categories} onEdit={onEdit} onDelete={onDelete} />
          ))
        )}
      </div>
    </div>
