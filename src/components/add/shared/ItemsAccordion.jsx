...
export function ItemsAccordion({ items, onToggleType, onEditAmount, onEditQuantity, totalAmount, categories, onToggleCategory }) {
  ...
  const [editMode,   setEditMode]  = useState(false);   // カテゴリー変更モード
  const [selected,   setSelected]  = useState(new Set()); // 選択中の品目index
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <p className="text-xs font-semibold text-indigo-600">
                      {selected.size === 0 ? "変更する品目を選択" : `${selected.size}件を選択中`}
                    </p>
                    <div className="flex gap-2">
                      <button onClick={() => setSelected(new Set(items.map((_, i) => i)))}
                        className="text-xs text-gray-500 border border-gray-200 px-2 py-1 rounded-lg">全選択</button>
                      <button onClick={() => { setEditMode(false); setSelected(new Set()); }}
                        className="text-xs text-gray-400 border border-gray-200 px-2 py-1 rounded-lg">キャンセル</button>
                    </div>
                  </div>
                  {selected.size > 0 && (
          ...
          <div className="divide-y divide-gray-50">
            {items.map((item, i) => (
              <div key={i} className={`px-4 py-2.5 ${
                editMode && selected.has(i) ? "bg-indigo-50" :
