...
export function AddPage({ categories, existingTransactions, allRules, learnedRules, onAdd, onLearnRule }) {
  ...
  const handleDupDecide = (decision) => {
    if (decision==="skip") { setMode("select"); setPendingTx(null); }
...
// 複数枚を順番に処理する
  ...
  const startOcr = async (imageFile) => {
    ...
    try {
      const { text, confidence } = await runTesseract(imageFile, setOcrProgress);
      ...
      <div className="px-4 py-5">
        {ocrStep==="upload" && (
          <div className="space-y-4">
            ...
            <button onClick={() => ocrFileRef.current?.click()}
              ...
              <span className="text-sm font-semibold text-gray-600">画像を選択（ギャラリー）</span>
            </button>
        ...
        {ocrStep==="processing" && (
          <div className="text-center space-y-4 py-8">
            ...
            <p className="text-sm font-semibold text-gray-700">文字を認識中...</p>
            <p className="text-xs text-gray-400">初回は30秒ほどかかる場合があります</p>
                ...
                <>
                  ...
                  {ocrConfidence<60 && (
                    <div className="mt-2 p-2 bg-rose-50 rounded-lg border border-rose-200 space-y-1">
                      ...
                      <p className="text-xs text-rose-500">• 金額を必ず確認してください</p>
                      <p className="text-xs text-rose-500">• 店舗名を必ず確認してください</p>
                    </div>
                  )}
                </>
              )}
        ...
        {csvStep==="preview" && (
          <div className="space-y-3">
            ...
            <div className="bg-white rounded-xl overflow-hidden border border-gray-100">
              {csvRows.map((r,i) => (
                <div key={i} className={`flex items-center gap-3 px-4 py-3 border-b border-gray-50 last:border-b-0 ${r.isDuplicate?"bg-amber-50":""}`}>
                  ...
                  <p className={`text-sm font-bold flex-shrink-0 ${r.type==="income"?"text-emerald-500":"text-rose-500"}`}>
